...
export default function Circulars() {
  ...
  useEffect(() => {
    ...
    // Filter for tutor audience
    setCirculars(all.filter(c => c.audience === 'all' || c.audience === 'tutors'));
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div>
          <h1 className="text-3xl font-bold italic">Tutor Announcements 📣</h1>
          <p className="text-muted-foreground font-medium">Important updates for Class Tutors and Coordinators</p>
        </div>
        ...
        {filteredCirculars.length > 0 ? filteredCirculars.map((notice, idx) => (
          ...
          >
            ...
            <div className="text-center py-20 bg-muted/20 border-2 border-dashed border-white/5 rounded-2xl">
                ...
                <p className="text-muted-foreground font-black uppercase tracking-widest text-[10px]">No circulars found for tutors.</p>
            </div>
        )}
      </div>
    </div>
