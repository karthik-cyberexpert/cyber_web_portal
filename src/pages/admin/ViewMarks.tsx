...
const getStatusBadge = (status: string) => {
  switch (status) {
    ...
    case 'pending_tutor': return 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30';
...
export default function ViewMarks() {
  ...
  useEffect(() => {
    ...
    fetchSubjects();
    fetchData();
  }, [activeTab, selectedSemester, selectedBatch, selectedSection]); // Refresh when major filters change
  ...
  const fetchSubjects = async () => {
    try {
        ...
        if (res.ok) {
            ...
            // Create a map of subjectId/code to semester
  ...
  const fetchData = async () => {
    ...
    try {
        const token = localStorage.getItem('token');
        // Use approval-status endpoint as base, it returns all submissions. 
        
        let url = `${API_BASE_URL}/marks/approval-status`;
        // Optimization: backend might not support filtering yet via query params on this endpoint,
        // so we fetch all and filter client side as per ApproveMarks.
