...
import { FileText, Download, Search, Filter, Calendar as StreamlineCalendar, User as UserIcon, BookOpen, GraduationCap } from 'lucide-react';
...
export default function LeaveReport() {
    ...
    useEffect(() => {
        if (isAdmin) {
            ...
            // Tutors might have their batch/section pre-selected from their profile
            fetchTutorDetails();
        }
    }, [user]);
    ...
    const fetchTutorDetails = async () => {
        try {
            const res = await fetch(`${API_BASE_URL}/academic/tutor/details`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            if (res.ok) {
                const data = await res.json();
                // If tutor is tied to a batch/section, we can use that
                // For now, let's assume we just allow semester selection as requested
            }
        } catch (e) {
            console.error("Tutor details fetch failed");
        }
    };
    ...
    const fetchReport = async () => {
        ...
        setIsLoading(true);
        try {
            // In a real scenario, use a report-specific endpoint
            // For this implementation, we fetch admin leave requests and filter client-side
            const res = await fetch(`${API_BASE_URL}/leave/admin`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            if (res.ok) {
                const data = await res.json();
                // Map and filter data
                const mapped: LeaveReportEntry[] = data.map((r: any) => ({
                    id: r.id,
                    studentName: r.user_name || 'Unknown',
                    rollNo: r.roll_number || 'N/A',
                    type: r.category || 'Leave',
    ...
    const handleExport = () => {
        ...
        XLSX.writeFile(wb, `Leave_Report_Sem_${selectedSemester}.xlsx`);
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Generate and export leave records</p>
                </div>
