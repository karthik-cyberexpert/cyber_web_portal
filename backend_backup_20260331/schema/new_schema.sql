-- Database Schema v1.8.0 (Cumulative Audit Fix)
...
CREATE TABLE `academic_years` (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
    `name` VARCHAR(50) NOT NULL, -- Modified from year_range for better naming
...
CREATE TABLE `batches` (
    ...
    `department_id` INT DEFAULT 1, -- Optional, single-department system
    `academic_year_id` INT, -- Link to academic year
    `start_year` YEAR NOT NULL,
    `end_year` YEAR NOT NULL,
...
CREATE TABLE `marks` (
    ...
    `student_id` INT NOT NULL,
    `subject_id` INT NOT NULL,
    `schedule_id` INT NOT NULL, -- Changed from exam_id
...
-- -----------------------------------------------------------------------------
-- 7. Communication & Other Features
-- -----------------------------------------------------------------------------
...
CREATE TABLE `feedback_answers` (
    ...
    `answer_text` TEXT,
    `rating` INT,
    `target_id` INT, -- ID of Faculty or Subject being rated
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
-- -----------------------------------------------------------------------------
-- 8. Seed Data (Essential for Fresh Install)
-- -----------------------------------------------------------------------------
...
INSERT INTO `departments` (name, code) VALUES ('Computer Science and Engineering (Cyber Security)', 'CSE-CS');
SET @DEPT_ID = LAST_INSERT_ID();

-- 3. Seed Essential Academic Data
INSERT INTO `academic_years` (name, start_year, end_year, is_active) VALUES ('2023-2024', 2023, 2024, TRUE);
SET @AY_ID = LAST_INSERT_ID();

INSERT INTO `batches` (name, academic_year_id, department_id, start_year, end_year, sections_count, current_semester, semester_dates_pending) 
VALUES ('2023-2027 Batch', @AY_ID, @DEPT_ID, 2024, 2028, 1, 1, TRUE);
SET @BATCH_ID = LAST_INSERT_ID();

INSERT INTO `sections` (batch_id, name, capacity) VALUES (@BATCH_ID, 'A', 60);
